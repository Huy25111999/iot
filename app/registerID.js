    ...
    $.ajax({
        ...
        success:function (res) {
            ...
            $.ajax({
                ...
                success: function (res) {
                    console.log(res);
                    var data_register = res.content.data.data.set_rf.rfid;
                    var idDK = "";

                    if (data_register.length == 0) {
                        console.log("Null");
                    } else {
                        for (var i = 0; i < data_register.length; i++) {
                            idDK += '<tr>';
                            idDK += '<td>' + (i + 1) + ". " + data_register[i] + '</td>';
                            idDK += '</tr>';
                        }
                    }
                    var idMissing = "";
                    ...
                    countID += '<td>' + data_register.length + '</td>';
...
function listID() {
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            var  dataTable = '<tr>';
            dataTable +='<td id="idHienTai" class = "id-rfid">'+'<table class = "table-content" id ="current_id"><tbody>'+ 1+'</tbody></table>'+'</td>';
            dataTable +='<td id="idDK" class = "id-rfid" ><table class = "table-content" id ="registered_id"><tbody >'+ idDK +'</tbody></table></td>';
            dataTable +='<td id="idMissing" class = "id-rfid"><table class = "table-content" id ="missing_id"><tbody >'+ 1+'</tbody></table></td>';
            dataTable +='<td id="idLa" class = "id-rfid"><table class = "table-content" id ="unknown_id"> <tbody >'+1 +'</tbody></table></td>';
            dataTable += '</tr>';
